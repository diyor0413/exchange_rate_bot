...
async def get_currency_exchange():
    ...
    async with aiohttp.ClientSession() as session:
        response = await session.get(url="https://bank.uz", headers=headers)
        soup = BeautifulSoup(await response.text(), 'lxml')

        currency_rate_content = soup.find_all("div", class_=' main_page')
        print(currency_rate_content)
        currency_dict = {}
        for currency_r in currency_rate_content:
            currency_name = 'EUR'
            currency_purchase = [item.text.strip() for item in currency_r.find(class_='bc-inner-block-left')]
        ...
        with open('euro_exchange_rate.json', 'w', encoding='utf-8') as file:
